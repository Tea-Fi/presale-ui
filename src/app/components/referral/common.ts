...
export const emptyStat = () =>
  ({ purchases: 0, soldInUsd: 0n, tokensSold: 0n }) as ReferralStats;
...
export function factorStats(
  ...
  return {
    ...
    tokensSold: factorTokens ? a.tokensSold * factor : a.tokensSold,
  };
}

export function subtreeSum(
  ...
  const stat = nodeLogs
    ...
    .reduce((acc, e) => {
      const rootLog = rootLogs?.find((x) => x.blockNumber === e.blockNumber);
...
export async function getReferralAmounts(
  referralId: number,
  chainId: number,
...
function mapLogs(referral: Referral, logs: EventLogWithTimestamp[]) {
  ...
  return logs.map((log) => {
    const fee = percentages.find((x) => x.date > log.time)?.fee ?? referral.fee;
...
export function calculateEarningByToken(
  tree: Referral,
  logs: EventLogWithTimestamp[],
) {
  ...
  while (queue.length > 0) {
    ...
    queue.push(
      ...Object.keys(current.subleads ?? {}).map((x) => current.subleads?.[x]!),
    );
  }

  const ids = new Set(nodes.map((x) => x.id) ?? []);
  const relevantLogs = logs.filter((x) =>
    ids.has((x.args as any)["referrerId"] as number),
  );
  ...
  const tokens = [
    ...new Set(relevantLogs.map((x) => (x.args as any)["tokenSold"])),
  ];
  const commission: Record<string, ReferralStats> = {};

  for (const token of tokens) {
    const tokenLogs = relevantLogs.filter(
      (x) => getArg<string>(x, "tokenSold") === token,
    );

    commission[token] = calculateCommission(tree, tokenLogs, {
      factorTokens: true,
      leavePrecision: true,
    });
  }

  ...
  useEffect(() => {
    handleOptionsInfo().then((data: any) => {
      ...
      setTotalSoldTeaPerAccount(
        Number(calculatedBalances.toFixed(2)).toLocaleString("en-US"),
      );
      setTgeInfos(newInfo);
    });
  ...
  return (
    <div className="claim flex flex-col mx-auto">
      ...
      <span className="text-center">
        Total Bought:{" "}
        {totalSoldTeaPerAccount === undefined ? (
          <Spinner />
        ) : (
            ...
            <Card key={index} className="size-64">
              ...
              <CardDescription className="flex flex-col gap-3">
                <span className="text-lg">
                  {projectInfo.balance !== null ? (
                    projectInfo.balance.toLocaleString("en-US")
                  ) : (
                    <Spinner />
                  )}{" "}
                  $TEA
